        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/performance/display/list">业绩展示</a></li>
                </ul>
            </div>
        </div>
        <div class="ibox-content" style="border-color: transparent">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/performance/display/list">
            ...
            <div class="example-wrap">
                <div class="example">
				<@shiro.hasPermission name="performance:display:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>

				<@shiro.hasPermission name="performance:display:delete">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <script id="datalist" type="text/html">
                            ...
                            <tr class="long-td">
                                ...
                                <td>
                                    {{# if(d[i].type==1){ }}
                                    <span style="background-color: #dd9222;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">酒店办公类</span>
                                    {{# }else if(d[i].type==2){ }}
                                    <span style="background-color: #000066;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">学校医院类</span>
                                    {{# }else if(d[i].type==3){ }}
                                    <span style="background-color: #1AA094;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">商贸住宅类</span>
                                    {{# }else if(d[i].type==4){ }}
                                    <span style="background-color: #EF5352;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">展览展示类</span>
                                    {{# }else { }}
                                    <span style="background-color: #00bbee;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">建筑幕墙类</span>
                                    {{# } }}
                                ...
                                <td>{{d[i].describe}}</td>
                                <td>
								<@shiro.hasPermission name="performance:display:edit">
                                    <a href="${basePath}/performance/display/edit/{{d[i].id}}"  class="btn btn-primary  btn-xs">
...
<script type="text/javascript">
    ...
    function Ajaxpage(curr){
        ...
        $.ajax({
            url: "${basePath}/performance/display/list",
    ...
    function add(){
        window.location.href="${basePath}/performance/display/add";

    }
    function edit(id){
        window.location.href="${basePath}/performance/display/edit/"+id;
    ...
    function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/performance/display/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
