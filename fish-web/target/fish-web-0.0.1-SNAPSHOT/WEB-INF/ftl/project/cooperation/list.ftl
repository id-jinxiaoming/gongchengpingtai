        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/project/cooperation/list">项目合作列表</a></li>
                </ul>
            </div>
        </div>
	    <div class="ibox-content" style="border-color: transparent">
	        ...
	        <div class="row">
	            <div class="col-sm-12">  
	                ...
	                <form name="admin_list_sea" class="form-search" method="get" action="${basePath}/project/cooperation/list">
	                    <div class="col-sm-3" style="padding-right: 0px;padding-left: 0px;">
	                        <div class="input-group">
	                            <input type="text" id="key" class="form-control" name="key" value="${key?default('')}" placeholder="输入项目名称" />
		            ...
		            <table class="table table-bordered table-hover">
		                ...
		                {{# for(var i=0;i<d.length;i++){  }}
							<tr class="long-td">
                                ...
                                <td>{{d[i].id}}</td>
                                <td>{{d[i].entryName}}</td>
                                <td>{{d[i].projectCost}}</td>
                                <td>{{d[i].itemCategory}}</td>
                                <td>{{d[i].itemAddress}}</td>
                                <td>{{d[i].cooperationProject}}</td>
                                <td>{{d[i].otherInstructions}}</td>
								 <td>{{d[i].createTime}}</td>
                                <td>

								<@shiro.hasPermission name="project:cooperation:show">

                                    <button type="button" class="btn btn-primary  btn-xs" onclick="show('{{d[i].userId}}')"><i class="fa fa-paste"></i>申请人信息</button>
								</@shiro.hasPermission>
								<@shiro.hasPermission name="project:cooperation:delete">
...
<div class="spiner-example">
	...
	function Ajaxpage(curr){
		$.ajax({
			url: "${basePath}/project/cooperation/list",
    ...
    function show(id){
        window.location.href="${basePath}/project/cooperation/show/"+id;
    ...
    function del(id){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/project/cooperation/delete",
                data:"id="+id,
                type: "POST",
                dataType:'json',
