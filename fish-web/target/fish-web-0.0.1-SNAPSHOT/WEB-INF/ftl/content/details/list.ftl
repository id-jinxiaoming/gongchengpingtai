        ...
        <div class="ibox-title">
            <div class="builder-tabs builder-form-tabs">
                <ul class="nav nav-tabs">
                    <li class="active"><a href="${basePath}/content/details/list">列表展示</a></li>
                </ul>
            </div>
        </div>
        <div class="ibox-content" style="border-color: transparent">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <form id="myForm" name="admin_list_sea" class="form-search" method="post" action="${basePath}/content/details/list">
            ...
            <div class="example-wrap">
                <div class="example">
				<@shiro.hasPermission name="content:details:add">
                    <button type="button" class="btn btn-outline btn-primary" onclick="add()">创建</button>
				</@shiro.hasPermission>

				<@shiro.hasPermission name="content:details:delete">
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <script id="datalist" type="text/html">
                            ...
                            <tr class="long-td">
                                ...
                                <td>
                                    {{# if(d[i].type==1){ }}
                                    <span style="background-color: #dd9222;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">新闻快讯</span>

                                    {{# }else if(d[i].type==2){ }}
                                    <span style="background-color: #1945de;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">微课堂</span>
                                    {{# }else if(d[i].type==3){ }}
                                    <span style="background-color: #1AA094;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">内部公告</span>
                                    {{# }else if(d[i].type==4){ }}
                                    <#--<span style="background-color: #EF5352;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">管理办法</span>
                                    {{# }else { }}
                                    <span style="background-color: #00bbee;color: #ffffff;border-radius: 5px;font-size: 20px;padding: 5px; ">办公流程</span>-->
                                    {{# } }}
                                ...
                                <td>
								<@shiro.hasPermission name="content:details:edit">
                                    <a href="${basePath}/content/details/edit/{{d[i].id}}"  class="btn btn-primary  btn-xs">
...
<script type="text/javascript">
    ...
    function Ajaxpage(curr){
        ...
        $.ajax({
            url: "${basePath}/content/details/list",
    ...
    function add(){
        window.location.href="${basePath}/content/details/add";

    }
    function edit(id){
        window.location.href="${basePath}/content/details/edit/"+id;
    ...
    function DelAll(){
        ...
        layer.confirm('确认删除吗?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                url: "${basePath}/content/details/delete",
                data:"id="+checkArray,
                type: "POST",
                dataType:'json',
